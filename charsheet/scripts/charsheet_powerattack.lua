...
function onSourceUpdate()
	...
	-- DOOM True 20
	if nType == 0 then -- True20 melee
		nValue = nValue + DB.getValue(nodeWin, "...attackbonus.melee.total", 0);
	end
	if nType == 1 then -- True20 ranged
		nValue = nValue + DB.getValue(nodeWin, "...attackbonus.ranged.total", 0);
    end
	if nType == 2 then -- True20 grapple
		nValue = nValue + DB.getValue(nodeWin, "...attackbonus.grapple.total", 0);
    end
	-- Doom True20 todo - add mental?
